...
export const INITIAL_RESUME = {
  personal: {
    ...
    summary:  "Full-stack engineer with 5+ years building scalable web applications. Passionate about clean code, great UX, and shipping products people love.",
  },
  education: [
    {
      id: "edu-1", institution: "UC Berkeley",
      degree: "Bachelor of Science", field: "Computer Science",
      startDate: "Aug 2015", endDate: "May 2019", gpa: "3.8",
    },
  ],
  experience: [
    {
      id: "exp-1", company: "Stripe", position: "Senior Software Engineer",
    ...
    {
      id: "exp-2", company: "Airbnb", position: "Software Engineer",
      startDate: "Jun 2019", endDate: "Dec 2021", current: false,
  ...
  skills: ["TypeScript", "React", "Node.js", "Python", "PostgreSQL", "Redis", "Docker", "AWS", "GraphQL", "Git"],
  projects: [
    {
      id: "proj-1", name: "OpenMetrics Dashboard",
...
export const TEMPLATES = [
  ...
  { id: "ats",     label: "ATS ✓",   description: "ATS Friendly — plain text, single column" },
];
...
export function ResumeProvider({ children }) {
  ...
  const setPersonal   = useCallback((d) => dispatch({ type: "SET_PERSONAL",   payload: d }), []);
  const setEducation  = useCallback((d) => dispatch({ type: "SET_EDUCATION",  payload: d }), []);
  const setExperience = useCallback((d) => dispatch({ type: "SET_EXPERIENCE", payload: d }), []);
  const setSkills     = useCallback((d) => dispatch({ type: "SET_SKILLS",     payload: d }), []);
  const setProjects   = useCallback((d) => dispatch({ type: "SET_PROJECTS",   payload: d }), []);
